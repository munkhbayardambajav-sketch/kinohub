...
app.get('/admin/debug-token1', requireAdmin, async (req, res) => { try { const tok = process.env.FB_PAGE_TOKEN_1; const meR = await fetch('https://graph.facebook.com/v19.0/me?access_token=' + tok); const me = await meR.json(); res.json({ me, expectedPageId: process.env.FB_PAGE_ID_1, tokenPrefix: (tok||'').slice(0,12) }); } catch (err) { res.status(500).json({ error: err.message }); } });
initDb().then(() => {
