...
app.get('/admin/debug-comments', requireAdmin, async (req, res) => { try { const pageId = req.query.pageId || '945425315317183'; const pageToken = getPageToken(pageId); const vr = await fetch('https://graph.facebook.com/v19.0/' + pageId + '/videos?fields=id,description&limit=5&access_token=' + pageToken); const vd = await vr.json(); const out = []; for (const v of (vd.data || [])) { const cr = await fetch('https://graph.facebook.com/v19.0/' + v.id + '/comments?fields=message,created_time&limit=50&access_token=' + pageToken); const cd = await cr.json(); out.push({ videoId: v.id, commentsError: cd.error || null, comments: (cd.data || []).map(c => ({ message: c.message, created_time: c.created_time })) }); } res.json({ videosError: vd.error || null, videoCount: (vd.data || []).length, out }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/admin/debug-pages', requireAdmin, async (req, res) => { try { const ids = [process.env.FB_PAGE_ID_1, process.env.FB_PAGE_ID_2].filter(Boolean); const out = []; for (const id of ids) { const tok = getPageToken(id); const r = await fetch('https://graph.facebook.com/v19.0/' + id + '?fields=name&access_token=' + tok); const d = await r.json(); out.push({ id, name: d.name || null, error: d.error || null }); } res.json(out); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/admin/debug-token2', requireAdmin, async (req, res) => { try { const tok = process.env.FB_PAGE_TOKEN_2; const meR = await fetch('https://graph.facebook.com/v19.0/me?fields=id,name&access_token=' + tok); const me = await meR.json(); const accR = await fetch('https://graph.facebook.com/v19.0/me/accounts?fields=id,name&access_token=' + tok); const acc = await accR.json(); res.json({ me, accounts: acc }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/admin/debug-token1', requireAdmin, async (req, res) => { try { const tok = process.env.FB_PAGE_TOKEN_1; const meR = await fetch('https://graph.facebook.com/v19.0/me?fields=id,name&access_token=' + tok); const me = await meR.json(); const accR = await fetch('https://graph.facebook.com/v19.0/me/accounts?fields=id,name&access_token=' + tok); const acc = await accR.json(); res.json({ me, accounts: acc }); } catch (err) { res.status(500).json({ error: err.message }); } });
initDb().then(() => {
